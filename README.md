...
flowchart LR
    ...
    subgraph app["`**python_openrecon_server/apps/app/your_module.py**`"]
...
All Python dependencies are declared in [`pyproject.toml`](https://github.com/benoitberanger/python-openrecon-server/blob/main/pyproject.toml). Versions are pinned
to match the base Docker image (see [`MRD.Dockerfile`](https://github.com/benoitberanger/python-openrecon-server/blob/main/MRD.Dockerfile)) so local runs and the
...
```
By default, it build the `demo` directory. 
...
Create a new directory in the `python_openrecon_server/apps/` directory containing a Python file. It must expose a function with this prototype:
...
This mode allows you to inspect the real metadata of images coming directly 
from the scanner for a given sequence, which is valuable when writing or 
debugging a new processing module, since this metadata can behave 
differently than what you get from DICOM-converted test files. And, combined 
with `--savedata`, it also allows you to run OpenRecon manually on the 
magnet just to capture realistic MRD `.h5` files for a given sequence, 
without running any actual processing.
...
| `--dirname`  | `demo`                  | Directory containing the processing module     |
| `-H`         | `0.0.0.0`               | Host address                                   |
| `-p`         | `9002`                  | TCP port                                       |
| `-v`         | —                       | Verbose logging                                |
| `--debug`    | —                       | Enable debug mode (passthrough, no processing) |
| `-l`         | —                       | Log file path                                  |
| `--saveNifti`| —                       | Convert to Nifti and save output data on disk  |
| `--savedata` | —                       | Save incoming data                             |
| `-S`         | `/tmp/share/saved_data` | Folder to save incoming data                   |
...
- **Python_openrecon_server/** :
    ...
    - **Apps/** :
        ...
        Default application directory for your application code. Contains the invert contrast example.
            - [**application.Dockerfile**](https://github.com/benoitberanger/python-openrecon-server/blob/main/python_openrecon_server/apps/demo/application.Dockerfile) : Application-specific Dockerfile, used as the base for the final image generated by `build.py`.
            - [**invertContrast_json_ui.json**](https://github.com/benoitberanger/python-openrecon-server/blob/main/python_openrecon_server/apps/demo/invertContrast_json_ui.json) : OpenRecon UI parameter definition for the contrast inversion application. Defines the parameters shown in the scanner interface.
            - [**invertContrast.py**](https://github.com/benoitberanger/python-openrecon-server/blob/main/python_openrecon_server/apps/demo/invertContrast.py): Example processing module. Inverts pixel contrast for each image type present in the dataset.
            - [**OpenReconSchema_1.1.0.json**](https://github.com/benoitberanger/python-openrecon-server/blob/main/python_openrecon_server/apps/demo/OpenReconSchema_1.1.0.json) : JSON schema used to validate the JSON UI file before building.

        - **Echo_sum/** : Example application that combines multi-echo magnitude images.
            - [**application.Dockerfile**](https://github.com/benoitberanger/python-openrecon-server/blob/main/python_openrecon_server/apps/echo_sum/application.Dockerfile) : Application-specific Dockerfile, used as the base for the final image generated by `build.py`.
            - [**echoSum_json_ui.json**](https://github.com/benoitberanger/python-openrecon-server/blob/main/python_openrecon_server/apps/echo_sum/echoSum_json_ui.json) : OpenRecon UI definition exposing the `EchoSumConfig` parameter to the scanner interface.
            - [**echoSum.py**](https://github.com/benoitberanger/python-openrecon-server/blob/main/echoSum/python_openrecon_server/apps/echoSum.py) : Processing module implementing simple sum and sum-of-squares.
            - [**OpenReconSchema_1.1.0.json**](https://github.com/benoitberanger/python-openrecon-server/blob/main/python_openrecon_server/apps/echo_sum/OpenReconSchema_1.1.0.json) : JSON schema used to validate the JSON UI file before building.
...
Tools to convert between DICOM and MRD format, required for local testing with `start_client.py`.
    ...
    - [**mrd2dicom.py**](https://github.com/benoitberanger/python-openrecon-server/blob/main/converter/mrd2dicom.py): Converts a processed MRD `.h5` file back to classic DICOM.
    - [**mrd2nifti.py**](https://github.com/benoitberanger/python-openrecon-server/blob/main/converter/mrd2nifti.py) : Converts an MRD `.h5` file (or an in-memory MRD image array) to NIfTI, auto-detecting extra dimensions (contrast, phase, repetition, set, average).
    - [**nifti2mrdImages.py**](https://github.com/benoitberanger/python-openrecon-server/blob/main/converter/nifti2mrdImages.py) : Rebuilds `ismrmrd.Image` objects from a NIfTI volume produced by an external tool, reusing geometry/metadata from the original MRD images.
...
- local test:
```bash
python main.py -c echoSum -d echo_sum/
```
- Package for use on the magnet:
```bash
python build.py --dirname echo_sum
```
