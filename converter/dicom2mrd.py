...
def main(args):
    ...
    for iSer in range(len(uSeriesNum)):
        ...
        for iImg in range(len(dsets)):
            ...
            tmpMrdImg.contrast              = tmpDset.EchoNumbers - 1
            # try:
            #     tmpMrdImg.repetition            = tmpDset.TemporalPositionIdentifier
            # except:
            #     pass

            # try:
            #     tmpMrdImg.average           = tmpDset.NumberOfAverages
            # except:
            #     pass
    ...
    if os.path.exists(args.outFile):
        os.remove(args.outFile)             # Delete the outfile, if it already exist because ismrmrd.Dataset is in append mode
